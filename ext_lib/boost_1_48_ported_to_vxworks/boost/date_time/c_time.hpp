...
namespace detail  // assumes private..
{
#if defined(BOOST_DATE_TIME_HAS_REENTRANT_STD_FUNCTIONS)
  inline std::tm* localtime_workaround(const std::time_t* input, std::tm* res)
  {
#if defined (__VXWORKS__)
    // for some reason VxWorks function returns int and not the result struct
    if (localtime_r(input, res) == OK)
    {
      res = 0;
    }
#else
    if (!localtime_r(input, res))
    {
      res = 0;
    }
#endif  // __VXWORKS__
...
inline std::tm* gmtime_workaround(const std::time_t* input, std::tm* res)
{
#if defined (__VXWORKS__)
    // for some reason VxWorks function returns int and not the result struct
    if (localtime_r(input, res) != OK)
  ...
  struct c_time {
    public:
 	...
 	if(!localtime_r(t,&tmp))
            ...
            *result = tmp;	
 	#else
        result = detail::localtime_workaround(t, result);
	#endif
        if (!result)
 	...
 	if(!gmtime_r(t,&tmp))
          ...
          *result = tmp;	
	#else
        result = detail::gmtime_workaround(t, result);
	#endif
        if (!result)
