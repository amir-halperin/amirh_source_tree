...
namespace PrivateNS
{
  ...
  inline int MakeDir(const char* dir, int params)
  {
#if defined(__VXWORKS__)
    return ::mkdir(dir);
#else
    return ::mkdir(dir, params);
#endif  // __VXWORKS__
  }
} // end of PrivateNS
...
inline bool create_directory(const char *path)
{  return PrivateNS::MakeDir(path, 0777) == 0 && ::chmod(path, 0777) == 0; }
...
{
#if defined(__VXWORKS__)
# define VXWORKS_OPEN_PARAM ,perm.get_permissions()
#else
# define VXWORKS_OPEN_PARAM
#endif  // __VXWORKS__
   ...
   while(1){
      ...
      if(ret >= 0){
         ...
         if((ret = ::open(name, (int)mode VXWORKS_OPEN_PARAM)) >= 0 || errno != ENOENT){
            break;
         }
      }
   }
...
{  
#if defined(__VXWORKS__)
# define  VXWORKS_OPEN_EXTRAC_PARAMS ,0666
#else
# define VXWORKS_OPEN_EXTRAC_PARAMS
#endif  // __VXWORKS__
   (void)temporary;
   return ::open(name, (int)mode VXWORKS_OPEN_EXTRAC_PARAMS);
}
...
{  
#if defined(__VXWORKS__)
# define VXWORKS_WRITE_CASTING_OP (char*)
#else
# define VXWORKS_WRITE_CASTING_OP
#endif  // __VXWORKS__
  return (ssize_t(numdata)) == ::write(hnd, VXWORKS_WRITE_CASTING_OP data, numdata);
}
