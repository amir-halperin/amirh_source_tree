...
inline T cyl_bessel_j_imp(T v, T x, const bessel_maybe_int_tag&, const Policy& pol)
{
   ...
   if((impl_abs_value<T>::get(ival) < 200) && (0 == v - ival))
   {
...
inline T cyl_neumann_imp(T v, T x, const bessel_maybe_int_tag&, const Policy& pol)
{
   ...
   if(floor(v) == v)
   {
      if((fabs(x) > asymptotic_bessel_y_limit<T>(tag_type())) && (fabs(x) > 5 * impl_abs_value<T>::get(v)))
      {
         T r = asymptotic_bessel_y_large_x_2(static_cast<T>(impl_abs_value<T>::get(v)), x);
