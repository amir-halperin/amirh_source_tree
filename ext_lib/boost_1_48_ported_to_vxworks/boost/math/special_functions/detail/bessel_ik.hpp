...
#include <boost/math/tools/detail/private_abs_select.hpp> // for some reason the compiler cannot choose the right abs function, this would help it
...
int temme_ik(T v, T x, T* K, T* K1, const Policy& pol)
{
    ...
    BOOST_ASSERT(impl_abs_value<T>::get(x) <= 2);
    BOOST_ASSERT(impl_abs_value<T>::get(v) <= 0.5f);
    ...
    b = exp(v * a);
    sigma = -a * v;
    c = impl_abs_value<T>::get(v) < tools::epsilon<T>() ?
       T(1) : T(boost::math::sin_pi(v) / (v * pi<T>()));
    d = impl_abs_value<T>::get(sigma) < tools::epsilon<T>() ?
        T(1) : T(sinh(sigma) / sigma);
    gamma1 = impl_abs_value<T>::get(v) < tools::epsilon<T>() ?
    ...
    for (k = 1; k < policies::get_max_series_iterations<Policy>(); k++)
    {
        ...
        sum += coef * f;
        sum1 += coef * h;
        if (impl_abs_value<T>::get(coef * f) < impl_abs_value<T>::get(sum) * tolerance) 
        { 
           break; 
        }
    }
...
int CF1_ik(T v, T x, T* fv, const Policy& pol)
{
    ...
    for (k = 1; k < policies::get_max_series_iterations<Policy>(); k++)
    {
        ...
        if (impl_abs_value<T>::get(delta - 1) <= tolerance) 
        { 
           break; 
        }
    }
...
int CF2_ik(T v, T x, T* Kv, T* Kv1, const Policy& pol)
{
    ...
    BOOST_ASSERT(impl_abs_value<T>::get(x) > 1);
    ...
    for (k = 2; k < policies::get_max_series_iterations<Policy>(); k++)     // starting from 2
    {
        ...
        BOOST_MATH_INSTRUMENT_VARIABLE(impl_abs_value<T>::get(S) * tolerance);
        if (impl_abs_value<T>::get(Q * delta) < impl_abs_value<T>::get(S) * tolerance) 
        { 
           break; 
        }
    }
