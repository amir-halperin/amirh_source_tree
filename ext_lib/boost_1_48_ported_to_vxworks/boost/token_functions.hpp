...
// MSVC does not like the following typename
  ...
  namespace Private
  {
    struct check_punct_wide
    {
      template<typename Char>
      static bool ret(Char c)
      {
        return static_cast< unsigned >(c) <= 255 && std::ispunct((int)c) != 0;
      }
      template<typename Char>
      static bool space(Char c)
      {
        return static_cast< unsigned >(c) <= 255 && std::isspace((int)c) != 0;
      }
    };
    
    struct check_punct_def
    {
      template<typename Char>
      static bool ret(Char c)
      {
        return std::ispunct((int)c) != 0;
      }
    ...
    template<typename Char>
    struct check_punct_select : boost::mpl::if_c<(sizeof(Char) == 1),
                                                  check_punct_def,
                                                  check_punct_wide>::type
    {
...
#else
      return Private::check_punct_select<char_type>::space(c);
#endif
    }
...
#else
      return Private::check_punct_select<char_type>::ret(c);
#endif
    }
  };
