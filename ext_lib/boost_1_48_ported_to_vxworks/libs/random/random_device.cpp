...
namespace
{
  // we need to over come the fact that in VxWorks open
  // is not working the same way as in Unix
  int inline open_file(const char* file, int flags, int mode)
  {
...
public:
  impl(const std::string & token) : path(token) {
    fd = open_file(token.c_str(), O_RDONLY, 0444);
    if(fd < 0)
      error("cannot open");
  }
