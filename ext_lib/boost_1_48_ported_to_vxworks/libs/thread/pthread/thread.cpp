...
#include <sched.h>  // for the schedule params
#ifdef __linux__
...
namespace boost
{
    namespace
    {
      inline void close_attributes_(thread_attributes::value_type* att)
      {
        if (att)
        {
      ...
      unsigned int this_thread_prio()
      {
        pthread_attr_t att;
        pthread_attr_init(&att);
        sched_param sp;
        pthread_attr_getschedparam(&att, &sp);
        unsigned int ret = sp.sched_priority;
        pthread_attr_destroy(&att);
        return ret;
      }
    } // end of local namespace
  
    thread_attributes::thread_attributes(const char* name, size_t stack_size, unsigned int prio) : data_(new value_type, close_attributes_)
    {
      memset(get(), 0, sizeof(pthread_attr_t));
      if (pthread_attr_init(get()) != 0)
      {
        data_.reset((pthread_attr_t*)0, close_attributes_); // and the attribute are no longer correct
      }
#if defined(__VXWORKS__)
      pthread_attr_setname(get(), (char*)name);
#endif  // __VXWORKS__
      if (!prio)
      {
        prio = this_thread_prio();  // use what this thread was set with
      }
      sched_param sp;
      memset(&sp, 0, sizeof(sp));
      sp.sched_priority = prio;
      pthread_attr_setschedparam(get(), &sp);
      
      pthread_attr_setstacksize(get(), stack_size);
    }
    
    thread_attributes::value_type* thread_attributes::get() const
    {
      return data_.get();
    }
...
//                 }
    ...
    void thread::start_thread()
    {
        ...
        int const res = pthread_create(&thread_info->thread_handle, thread_info->attributes(), &thread_proxy, thread_info.get());
        if (res != 0)
        {
            thread_info->self.reset();
...
#endif  // there is nano sleep in vxworks - just make sure ot let boost know
             if (false)
#endif  // force the use of nano sleep
            {
