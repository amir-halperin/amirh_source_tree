...
#     include <sysLib.h>  // for the tick to seconds
#     include <tickLib.h> // for the tick count
    // with vxworks we don't have this information
    // but since this is not valid for vxworks -
    // since we don't have any processes to measure 
    // time to, we can set all of them
    // to the same value
    // this information was taken from http://linux.die.net/man/2/times
       struct tms {
         ...
         tms(clock_t val) : tms_utime(val), tms_stime(val), tms_cutime(val), tms_cstime(val)
         {
         }
         
         tms& operator = (clock_t val)
         {
          *this = tms(val); // we can assign to single value on vxworks
          return *this;
         }
       };
#   endif // __VXWORKS__
...
namespace
{
  ...
  inline boost::int_least64_t get_ticks_factor()
  {
#if defined(__VXWORKS__)
    return sysClkRateGet(); // no errors
#else
    boost::int_least64_t val = ::sysconf(_SC_CLK_TCK);
    if (val < 0)
    {
      val = -1;
    }
  ...
  inline clock_t process_time(tms& tm)
  {
#if defined(__VXWORKS__)
    unsigned long ticks = tickGet();
    tm = ticks;   // we dont have good value for that (per task)
    return ticks;
#else
    return ::times(&tm);
#endif  // __VXWORKS__
...
# if defined(BOOST_POSIX_API)
                                     ...
                                     //  to nanoseconds; -1 if unknown
  {
    ...
    if (!tick_factor)
    {
      if ((tick_factor = get_ticks_factor()) <= 0)
        tick_factor = -1;
      else
      {
